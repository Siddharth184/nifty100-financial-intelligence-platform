...
def compute_leverage_kpis(row: Dict[str, Any], is_financial: bool = False) -> Dict[str, Any]:
    ...
    borrowings = row.get("borrowings", row.get("total_debt_cr", 0.0))
    total_equity = row.get("total_equity")
    operating_profit = row.get("operating_profit")
    interest = row.get("interest")
    sales = row.get("sales")
    total_assets = row.get("total_assets")
    cash = row.get("investments", 0.0) # Or cash equivalents if stored
    ...
    b_val = float(borrowings) if borrowings is not None and not pd.isna(borrowings) else 0.0
